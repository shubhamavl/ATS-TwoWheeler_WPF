    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void Window_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (ctrlPressed)
                {
                    ...
                    {
                        RequestStreamBtn_Click(sender, new RoutedEventArgs());
                        e.Handled = true;
                    }
                    else if (e.Key == Key.S)
                    {
        ...
        private void UpdateStreamingIndicators()
        {
            try
            {
                ...
                bool isStreamActive = _streamRunning;
                    ...
                    if (_currentADCMode == 0) // Internal ADC
                    {
                        if (message.Data?.Length >= 2)
                        {
                            ...
                            if (!_streamRunning)
                            {
                                ...
                                _logger.LogInfo($"Stream started (Internal ADC)", "CAN");
                            }
                            ...
                            if (!_streamRunning)
                            {
                                ...
                                UpdateStreamingIndicators();
                                UpdateDashboardMode();
                                _logger.LogInfo("Stream confirmed active via 0x200 (Internal)", "CAN");
                            }
                            
                            // Send to WeightProcessor
                            _weightProcessor.EnqueueRawData(rawADC16);
                        }
                    }
                    else // ADS1115
                    {
                        ...
                        {
                            // 4 bytes (signed -131072 to +131068 for 4 channels) - little-endian int32
                            int rawADC32 = BitConverter.ToInt32(message.Data, 0);
                            _totalRawADC = rawADC32;
                            _currentRawADC = _totalRawADC;
                            ...
                            if (!_streamRunning)
                            {
                                ...
                                UpdateDashboardMode();
                                UpdateWeightProcessorCalibration();
                                _logger.LogInfo($"Stream started (ADS1115 signed)", "CAN");
                            }
                            ...
                            if (!_streamRunning)
                            {
                                ...
                                UpdateStreamingIndicators();
                                UpdateDashboardMode();
                                _logger.LogInfo("Stream confirmed active via 0x200 (ADS1115)", "CAN");
                            }
                            
                            // Send to WeightProcessor
                            _weightProcessor.EnqueueRawData(rawADC32);
                        }
                    }
                    break;
        ...
        private void UpdateWeightDisplays()
        {
            try
            {
                ...
                var totalData = _weightProcessor.LatestTotal;
                
                // Get current calibration based on ADC mode
                ...
                // Get data for total weight
                _currentRawADC = totalData.RawADC;
                ...
                {
                    bool isTared = _tareManager.IsTared(_activeADCMode);
                ...
                if (_dataLogger.IsLogging && _streamRunning)
                {
                    double tareOffset = _tareManager.IsTared(_activeADCMode) 
                        ? _tareManager.GetOffsetKg(_activeADCMode) 
                        : 0.0;
                    
                    _dataLogger.LogDataPoint("Total", totalData.RawADC, totalData.CalibratedWeight, totalData.TaredWeight, 
        ...
        private LinearCalibration? GetCurrentCalibration()
        {
            ...
            return _activeADCMode == 0 ? _totalCalibrationInternal : _totalCalibrationADS1115;
        }

        private void LoadCalibrations()
        {
            // Load calibrations for total weight (Internal/ADS1115)
            ...
            // Update WeightProcessor with calibration for current active mode
            UpdateWeightProcessorCalibration();
        ...
        private void UpdateCalibrationMode(string mode)
        {
            ...
            if (_totalCalibrationInternal != null)
                _totalCalibrationInternal.Mode = calibrationMode;
            if (_totalCalibrationADS1115 != null)
                _totalCalibrationADS1115.Mode = calibrationMode;
            ...
            {
                if (_totalCalibrationInternal != null && (_totalCalibrationInternal.Segments == null || _totalCalibrationInternal.Segments.Count == 0))
                    _totalCalibrationInternal.BuildSegmentsFromPoints();
                if (_totalCalibrationADS1115 != null && (_totalCalibrationADS1115.Segments == null || _totalCalibrationADS1115.Segments.Count == 0))
                    _totalCalibrationADS1115.BuildSegmentsFromPoints();
            }
        ...
        /// Uses current system ADC mode for total weight calibration
        /// </summary>
        private void UpdateWeightProcessorCalibration()
        {
            // Use active ADC mode if stream is active, otherwise use current system ADC mode
            byte adcModeToUse = _streamRunning ? _activeADCMode : _currentADCMode;
            
            LinearCalibration? totalCal = adcModeToUse == 0 ? _totalCalibrationInternal : _totalCalibrationADS1115;
            _weightProcessor.SetCalibration(totalCal);
            
            // Reset filters when calibration changes
            _weightProcessor.ResetFilters();
            
            // Update ADC mode to ensure correct tare baseline is used
            // This prevents applying tare from wrong ADC mode when switching modes
            _weightProcessor.SetADCMode(adcModeToUse);
        }
        ...
        private void Tare_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!_streamRunning)
                {
                    MessageBox.Show("Please start a stream before taring.", "No Active Stream", 
                ...
                {
                    MessageBox.Show("Please calibrate first before taring.", "Calibration Required", 
                                  MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                _tareManager.TareTotal(currentCalibratedKg, _activeADCMode);
        ...
        private void ResetTare_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!_streamRunning)
                {
                    MessageBox.Show("Please start a stream before resetting tare.", "No Active Stream", 
                                  MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                
                // Use mode-specific reset (active ADC mode)
                _tareManager.ResetTotal(_activeADCMode);
                ...
                UpdateWeightDisplays();
                string modeText = _activeADCMode == 0 ? "Internal" : "ADS1115";
                MessageBox.Show($"Total weight ({modeText}) tare offset cleared successfully.\n\nReadings will no longer be compensated.", "Reset Complete", 
        ...
        private void Calibrate_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (!_streamRunning && _canService?.IsConnected == true)
                {
                    bool success = _canService.StartStream(_currentTransmissionRate);
                    if (success)
                    {
                        _streamRunning = true;
                        _activeADCMode = _currentADCMode;
                        _logger.LogInfo($"Auto-started stream for calibration at rate {_currentTransmissionRate}", "CAN");
                        UpdateStreamingIndicators();
                    ...
                    {
                        _logger.LogWarning($"Could not auto-start stream - calibration will use manual entry mode", "CAN");
                    }
                }
                
                // Get current ADC mode (or default to Internal)
                byte currentADCMode = _streamRunning ? _activeADCMode : _currentADCMode;
                ...
                var calibrationDialog = new CalibrationDialog(currentADCMode, calibrationDelayMs);
        ...
        private void RequestStreamBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (_streamRunning)
                {
                    ShowInlineStatus($"? Stream already running at {GetRateText(_currentTransmissionRate)}", true);
                    _logger.LogWarning("Attempted to start stream while already running", "CAN");
                    return;
                }

                bool success = _canService?.StartStream(_currentTransmissionRate) ?? false;
                FlashTxIndicator();
                ...
                if (success)
                {
                    ...
                    UpdateDashboardMode();
                    UpdateWeightProcessorCalibration();
                    _logger.LogInfo($"Started stream at rate {_currentTransmissionRate}", "CAN");
                    ShowInlineStatus($"? Stream started at {GetRateText(_currentTransmissionRate)}");
                }
                else
                {
                    _streamRunning = false;
                    _logger.LogError("Failed to start stream", "CAN");
                    ShowInlineStatus("? Failed to start stream", true);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"Stream start error: {ex.Message}", "CAN");
        ...
        private void InitializeApplication()
        {
            ...
            try
            {
                ...
                if (_settingsManager.Settings.EnableBootloaderFeatures)
                {
                    _bootloaderDiagnosticsService = new BootloaderDiagnosticsService();
                    _firmwareUpdateService = new FirmwareUpdateService(_canService);
                    _firmwareUpdateService.SetDiagnosticsService(_bootloaderDiagnosticsService);
                }
        ...
        {
            string message = 
                $"?? You are about to downgrade from version {currentVersion} to version {targetVersion}.\n\n" +
                ...
                $"?? Recommendation: Backup your settings and calibration data before proceeding.\n\n" +
            ...
            try
            {
                ...
                if (!File.Exists(updaterPath))
                {
                    ...
                    bool isDevelopment = 
                        ...
                        !File.Exists(System.IO.Path.Combine(appDir, "ATS_TwoWheeler_WPF.exe"));
                ...
                try
                {
                    string mainExeName = System.IO.Path.GetFileName(System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName ?? "ATS_TwoWheeler_WPF.exe");
        ...
        private void LoadAdvancedSettings()
        {
            try
            {
                ...
                // Bootloader features are now always enabled (accessible via Tools > Firmware Manager)
                ...
                // Bootloader features are now always enabled (accessible via Tools > Firmware Manager)
                // Setting is kept for compatibility but UI checkbox removed
                ...
                if (CalibrationOutlierThresholdSlider != null)
                {
                    ...
                    {
                        CalibrationOutlierThresholdValueTxt.Text = $"{outlierThreshold:F1} s";
                    }
                }
        ...
        private void ApplyAdvancedSettings()
        {
            try
            {
                ...
                // Bootloader features are now always enabled (accessible via Tools > Firmware Manager)
                // Ensure setting is enabled for compatibility
                _settingsManager.SetBootloaderFeaturesEnabled(true);
        ...
        private void CalibrationModeInfoBtn_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show(
                ...
                "Note: Mode applies to all calibrations (Total Weight, Internal/ADS1115).",
        ...
        private void CalibrationOutlierThresholdSlider_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            try
            {
                if (CalibrationOutlierThresholdValueTxt != null)
                {
                    ...
                    CalibrationOutlierThresholdValueTxt.Text = $"{threshold:F1} s";
                }
                ApplyAdvancedSettings();
            }
            catch (Exception ex)
        ...
        {
            // Firmware Manager is now accessed via Tools menu
            // This method is kept for compatibility but no longer needs to update UI visibility
            try
            {
                // Bootloader features are now accessed through Tools > Firmware Manager
                // No UI elements to show/hide in main window
            }
            catch (Exception ex)
            {
...
Controls visibility of streaming status indicators (green/gray dot and ""Active"" / ""Stopped"" text).
...
Most users should keep default values unless experiencing specific issues.";
        ...
        private void UpdateConnectionStatus(bool connected)
        {
            try
            {
                ...
                {
                    RequestStreamBtn.IsEnabled = connected;
                }
        ...
        {
            // Firmware controls removed from main window - now in BootloaderManagerWindow
            // This method kept for compatibility but no longer needs to update UI
        }
        ...
        private void OnClosing(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                _canService?.Disconnect();
                // Firmware update cancellation now handled in BootloaderManagerWindow
                if (_canService != null)
                {
                    _canService.DataTimeout -= OnDataTimeout;
                }
        ...
        {
            // Redirect to Firmware Manager - this handler kept for compatibility
            OpenFirmwareManager_Click(sender, e);
        }

        private void StartFirmwareUpdateBtn_Click(object sender, RoutedEventArgs e)
        {
            // Redirect to Firmware Manager - this handler kept for compatibility
            OpenFirmwareManager_Click(sender, e);
        }
        ...
        private void OpenFirmwareManager_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Close the Tools menu popup
                if (ToolsMenuPopup != null)
                {
                    ToolsMenuPopup.IsOpen = false;
                }

                if (!EnsureCanConnection("Firmware Manager"))
                    return;

                var managerWindow = new BootloaderManagerWindow(_canService, _firmwareUpdateService, _bootloaderDiagnosticsService);
                managerWindow.Owner = this;
                managerWindow.ShowDialog();
            }
            catch (Exception ex)
            {
                _logger.LogError($"Failed to open firmware manager: {ex.Message}", "FW");
                ShowInlineStatus($"Failed to open firmware manager: {ex.Message}", true);
            }
        }

        private void OpenBootloaderManagerBtn_Click(object sender, RoutedEventArgs e)
        {
        ...
        private TwoWheelerWeightWindow? _twoWheelerWindow;

        private void OpenTwoWheelerWindow_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Close popup
                ...
                if (_twoWheelerWindow != null && _twoWheelerWindow.IsLoaded)
                {
                    _twoWheelerWindow.Activate();
                    _twoWheelerWindow.Focus();
                    return;
                }

                _twoWheelerWindow = new TwoWheelerWeightWindow(_canService, _weightProcessor);
                _twoWheelerWindow.Owner = this;
                _twoWheelerWindow.Closed += (s, args) => { _twoWheelerWindow = null; };
                _twoWheelerWindow.Show();
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error opening Two Wheeler window: {ex.Message}", "UI");
                MessageBox.Show($"Error opening Two Wheeler window: {ex.Message}", "Error",
                              MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
            ...
            try
            {
                Dispatcher.Invoke(() =>
                {
                    ...
                    if (_streamRunning)
                    {
                        ...
                        _logger.LogWarning("Data timeout: Stopped stream due to no data received", "CAN");
                    }
        ...
        private void ResetCalibration_Click(object sender, RoutedEventArgs e)
        {
            if (!_streamRunning)
            {
                MessageBox.Show("Please start a stream before resetting calibration.", "No Active Stream", 
                              MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            
            ResetCalibration(_activeADCMode);
        }
        
        private void ResetCalibration(byte adcMode)
        {
            try
            {
        ...
        /// <summary>
        /// Update dashboard mode header to show current ADC mode
        /// </summary>
        ...
        private void HeaderRateCombo_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                if (HeaderRateCombo?.SelectedIndex >= 0)
                {
                    ...
                    // Graph window removed for ATS Two-Wheeler system
        ...
        private void LoadConfiguration()
        {
            try
            {
                string configPath = System.IO.Path.Combine(Environment.CurrentDirectory, "TwoWheeler_Config.json");
