...
3. **Create a ZIP file**:
   ...
   - Name it: `ATS_TwoWheeler_WPF_Portable_v1.0.zip` (or your version)
...
2. Run `ATS_TwoWheeler_WPF.exe`
3. Verify it works correctly
4. Check that it creates `Data/` and `Logs/` folders
5. Confirm that `ATS_TwoWheeler_Updater.exe` exists next to `ATS_TwoWheeler_WPF.exe`
...
- `ATS_TwoWheeler_WPF_Portable_v1.0.zip`
...
2. **Double-click** `ATS_TwoWheeler_WPF.exe`
...
2. Run `ATS_TwoWheeler_WPF.exe` directly from the USB
